...
{
    if (((float)a->numerator/(float)a->denominator) > ((float)b->numerator/(float)b->denominator)) return 1;
    if (((float)a->numerator/(float)a->denominator) == ((float)b->numerator/(float)b->denominator)) return 0;
    else return -1;
}
...
{
    return setFraction(((a->numerator * b->denominator) + (b->numerator * a->denominator)), (a->denominator * b->denominator));
}
...
{
    unsigned int d = gcd(val->numerator, val->denominator);
    val->numerator = val->numerator/d;
    val->denominator = val->denominator/d;
}
